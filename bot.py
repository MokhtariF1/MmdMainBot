...
async def message(event):
    ...
    if text == bot_text["account"]:
        ...
        if len(find_tests) == 0:
            ...
            url = "http://127.0.0.1:8000/get-service/?number=1"
            ...
            if user_can_test:
                ...
                if response["status"] == 200:
                    ...
                    cur.execute(f"DELETE FROM services WHERE test_num = {test_num}")
...
async def yes_wallet(event):
    ...
    url = f"http://127.0.0.1:8000/get-service/?number={service_num}"
...
async def serv_info_get(event):
    ...
    response = response.json()
    is_active = None
    if response["info"]['is_active'] is False:
        is_active = "غیر فعال"
    else:
        is_active = "فعال"
    full_text = f"""
...
async def one_ex_service(event):
    ...
    if is_ban:
        ...
        return
    client_id = event.data.decode().split(":")[1]
    buttons = []

    for key, value in config.one_member_names.items():

        buttons.append([Button.inline(value, data=str.encode("plan_ex_service:" + str(key) + ":" + str(client_id)))])
...
async def plan_ex_service(event):
    user_id = event.sender_id
    service_num = int(event.data.decode().split(":")[1])
    plan_id = config.plans_json[service_num]
    client_id = str(event.data.decode().split(":")[2])
    amount = config.amounts[service_num]
    ...
    keys = [
        ...
        [
            ...
            Button.inline(bot_text["pay_and_active_ex"], str.encode("buy_ex_wallet:" + str(service_num) + ":" + str(client_id)))
...
async def buy_ex_wallet(event):
    ...
    service_num = int(event.data.decode().split(":")[1])
    client_id = event.data.decode().split(":")[2]
    ...
    if user_inventory - int(find_amount) < 0:
        ...
        keys = [
            ...
            Button.inline(bot_text["yes"], str.encode("wallet_ex_yes:" + str(service_num) + ":" + str(client_id))),
...
async def wallet_ex_yes(event):
    ...
    print(client_id)
    print(plan_id)
    url = f"{config.API_ADDRESS}service-extension/?number={plan_id}&client_id={client_id}"
