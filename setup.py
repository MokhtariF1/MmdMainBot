...
cur.execute(
    "CREATE TABLE IF NOT EXISTS users(user_id, inventory, phone, can_test, is_ban, access_code, has_access, user_type, rep_code, send_notification)")
cur.execute(
...
# cur.execute(f"ALTER TABLE users ADD rep_code {None}")
# users = cur.execute("SELECT user_id FROM users").fetchall()
# for user in users:
#     user_id = user[0]
#     cur.execute(f"UPDATE users SET rep_code = 'normal' WHERE user_id = {user_id}")
#     db.commit()
